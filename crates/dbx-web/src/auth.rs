...
pub async fn login(State(state): State<Arc<WebState>>, Json(body): Json<LoginRequest>) -> Result<Response, StatusCode> {
    ...
    Ok((StatusCode::OK, [("set-cookie", cookie.as_str())], Json(serde_json::json!({"ok": true, "session": token})))
        .into_response())
}
...
pub fn session_token_from_headers(headers: &axum::http::HeaderMap) -> Option<String> {
    if let Some(header) = headers.get("x-dbx-session").and_then(|value| value.to_str().ok()) {
...
fn extract_session_token<B>(req: &Request<B>) -> Option<String> {
    if let Some(token) = session_token_from_headers(req.headers()) {
        return Some(token);
    }

...
impl AppState {
    ...
    async fn transport_layer_failover_attempts(
        ...
        let is_failover_group = proxy_refs.len() >= 2 && proxy_refs.iter().skip(1).all(|proxy| !proxy.enabled);
    ...
    pub async fn connection_host_port(
        ...
        for (index, (label, layers)) in attempts.into_iter().enumerate() {
            ...
            match self.start_transport_attempt(connection_id, &layers, &remote_host, remote_port).await {
                Ok(local_port) => {
                    if total > 1 {
    ...
    pub async fn reset_connection_transport_for_config(&self, connection_id: &str, config: &ConnectionConfig) {
        let existing_layer_count = {
            let configs = self.configs.read().await;
            configs.get(connection_id).map(|config| config.transport_layers.len()).unwrap_or(0)
        };
...
fn proxy_attempt_label(layers: &[TransportLayerConfig], ordinal: usize) -> String {
    let Some(TransportLayerConfig::Proxy(proxy)) =
        layers.iter().rev().find(|layer| matches!(layer, TransportLayerConfig::Proxy(_)))
    else {

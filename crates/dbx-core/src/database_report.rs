...
use crate::database_stats::{
    ...
    CatalogStatsExecutor, CatalogStatsScope, DatabaseStatsError, DatabaseStatsOptions,
};
...
pub async fn fetch_database_report(
    ...
    if db_type == "redis" || db_type == "mongodb" {
        let stats = fetch_database_stats(executor, config, options).await?;
    ...
    if let Some(comments_sql) = build_catalog_column_comments_sql(&db_type, &catalog_scope) {
        match executor.execute_query(&query_config, &database, &comments_sql, timeout_secs).await {
    ...
    if let Some(index_sql) = build_catalog_index_sql(&db_type, &catalog_scope) {
        match executor.execute_query(&query_config, &database, &index_sql, timeout_secs).await {

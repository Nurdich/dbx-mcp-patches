...
impl DbxMcpServer {
    ...
    async fn execute_query(&self, Parameters(request): Parameters<ExecuteQueryRequest>) -> CallToolResult {
        let session_id_opt = request.session_id.as_deref().map(str::trim).filter(|id| !id.is_empty());
        if let Some(session_id) = session_id_opt {
            ...
            let session = match session {
                Some(session) if session.connection_id == connection.id => session,
                Some(_) => {
                    return tool_error(
...
impl DbxMcpServer {
    ...
    async fn execute_query_on_connection(
        ...
        let _permissions = validate_sql_policy(connection, &policy, &database, &request.sql, false)?;

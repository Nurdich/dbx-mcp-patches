...
export default withEnglishFallback({
  ...
  settings: {
    ...
    remoteApiTitle: "远程 DBX API",
    remoteApiDescription: "让本机 UI 连接远程 dbx-web API（连接存储与查询在服务器执行）。与 CLI/MCP 的 DBX_WEB_URL 同理。",
    remoteApiUrl: "API 基址",
    remoteApiUrlPlaceholder: "https://dbx.example.com",
    remoteApiPassword: "Web 登录密码",
    remoteApiPasswordHint: "与远程 Web 登录密码 / DBX_WEB_PASSWORD 相同。",
    remoteApiEnvOverride: "当前由环境变量覆盖（VITE_DBX_WEB_URL）。清除环境变量后可使用下方设置。",
    remoteApiSave: "保存并重载",
    remoteApiClear: "改回本地后端",
    remoteApiSaved: "远程 API 已保存，正在重载…",
    remoteApiCleared: "已切回本地后端，正在重载…",
    remoteApiUrlInvalid: "请输入合法的 http(s) URL，或留空使用本地后端。",
    mcpChecking: "检查中",

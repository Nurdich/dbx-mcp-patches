...
export default withEnglishFallback({
  ...
  settings: {
    ...
    remoteApiTitle: "遠端 DBX API",
    remoteApiDescription: "讓本機 UI 連線遠端 dbx-web API（連線儲存與查詢在伺服器執行）。與 CLI/MCP 的 DBX_WEB_URL 同理。",
    remoteApiUrl: "API 基址",
    remoteApiUrlPlaceholder: "https://dbx.example.com",
    remoteApiPassword: "Web 登入密碼",
    remoteApiPasswordHint: "與遠端 Web 登入密碼 / DBX_WEB_PASSWORD 相同。",
    remoteApiEnvOverride: "目前由環境變數覆寫（VITE_DBX_WEB_URL）。清除環境變數後可使用下方設定。",
    remoteApiSave: "儲存並重載",
    remoteApiClear: "改回本機後端",
    remoteApiSaved: "遠端 API 已儲存，正在重載…",
    remoteApiCleared: "已切回本機後端，正在重載…",
    remoteApiUrlInvalid: "請輸入合法的 http(s) URL，或留空使用本機後端。",
    mcpChecking: "檢查中",
